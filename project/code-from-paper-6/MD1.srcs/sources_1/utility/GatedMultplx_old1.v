...
//Gated multiplexer with nPort input ports, each with width nByte. Inputs are buffered in small fifo (depth 2) upon tx_req_port, and sent out
//with different priority (port 0; highest priority, port nPort-1: lowest priority
//
//input:
...
always @(posedge IOclk) begin
  ...
  for(i=1;i<nPort;i=i+1) begin                 //from remaining ports, port 1 has highest priority, all subsequent ports have succesively lower priority
    error_port <= (tx_req_port[i]==1'b1)&&(tx_req_reg[i-1]==1'b1);  
    if(tx_req_reg[i-1]==1'b0) begin                  
      ...
      dataout         <= data_port[i*nByte*8+:nByte*8];
      tx_req          <= 1'b1;
      tx_req_reg[i-1] <= 1'b0;  
      end 
    end
