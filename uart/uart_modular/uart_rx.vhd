...
architecture Behavioral of uart_rx is
    ...
    signal rx_valid_cnt : integer range 0 to 2 := 0;
    ...
    begin
        if rising_edge(Clk) then
            if Rst = '0' then
                ...
                -- manage rx_valid pulse stretching (hold for a couple of clocks)
                if rx_valid_cnt > 0 then
                    if rx_valid_cnt = 1 then
                        rx_valid_s <= '0';
                    end if;
                ...
                case state is
                    ...
                    when STOP =>
                        if baud_tick = '1' then
                            ...
                            if rx_sync_1 = '1' then
                                rx_data_s <= shift_reg;
                                rx_valid_s <= '1'; -- set valid now
                                rx_valid_cnt <= 2; -- hold for current + next cycles
                            end if;
                            state <= IDLE;
                        end if;
