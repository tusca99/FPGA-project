...
\begin{frame}{What is Percolation?}
  ...
  \begin{columns}
    ...
    \column{0.5\textwidth}
    \centering
    \begin{tikzpicture}[x=0.06\textwidth, y=0.06\textwidth, every node/.style={minimum size=0.06\textwidth, inner sep=0pt}]
      \pgfmathsetseed{42}
      \foreach \x in {0,...,7}
        \foreach \y in {0,...,7} {
          \pgfmathparse{random(0,100)<58?1:0}
          \ifnum\pgfmathresult=1
            \node[fill=blue!50, draw=blue!70, thick] at (\x,\y) {};
          \else
            \node[fill=gray!20, draw=gray!40, thick] at (\x,\y) {};
          \fi
        }
      % Spanning path (illustrative — explained verbally)
      \draw[red, ultra thick] (1,7) -- (1,6) -- (2,6) -- (2,5) -- (3,5)
            -- (3,4) -- (4,4) -- (4,3) -- (5,3) -- (5,2) -- (5,1) -- (5,0);
      \node[red, font=\small] at (4,7.5) {spanning cluster};
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Directed Percolation (DP)}
  ...
  \begin{columns}
    ...
    \begin{tikzpicture}[x=0.06\textwidth, y=0.06\textwidth, every node/.style={minimum size=0.06\textwidth, inner sep=0pt}]
      ...
      % Directed path (illustrative — explained verbally)
      \draw[red, ultra thick, ->] (2,7) -- (2,6) -- (3,6) -- (3,5) -- (4,5)
            -- (4,4) -- (4,3) -- (5,3) -- (5,2) -- (5,1) -- (5,0);
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Frontier Engine — Row-Wise Reachability}
  ...
  \begin{itemize}
    \item \alert{3-stage pipeline}: READY $\to$ COMPUTE $\to$ SAVE = 3 cycles/row
...
\begin{frame}{Three-Way Comparison}
  \begin{columns}
    ...
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.78\textwidth]{output/three_way_very_fine.png}\\
  ...
  \begin{itemize}
    ...
    \item Software and hardware agree within statistical error
  \end{itemize}
\end{frame}
...
\begin{frame}{Asymptotic Cycles-Per-Run Decomposition}
  ...
  \begin{itemize}
    ...
    \item Asymptote matches theoretical $3 \times S$ cycles/run
  \end{itemize}
\end{frame}
...
\begin{frame}{Pipeline Efficiency}
  ...
  \vspace{0.2cm}
  \begin{itemize}
    \item Measured: $C_{\text{core}} = 3.5 \times S + \text{const}$ (vs ideal $3 \times S$)
    \item \alert{Efficiency: 85--95\%} of theoretical maximum
    \item Excess per step: $\approx$0.5 cycles (host dispatch + UART driver)
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary of Results}
  \begin{table}
    ...
    \begin{tabular}{lcc}
      ...
      Cycles per run ($N{\times}N$) & $3N + 3$ & e.g. $N=64 \to \approx 195$ \\
      Pipeline efficiency & 85--95\% & Of theoretical $3\times S$ \\
