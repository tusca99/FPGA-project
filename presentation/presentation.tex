...
\draw[signal]
    (rng.east) --
    node[label, above=3pt] {site\_open}
    (core.west);

\draw[signal]
    (core.east) --
    node[label, above=3pt] {row}
    (frontier.west);

\draw[signal]
    (frontier.east) --
    node[label, above=3pt] {stats}
    (uart.west);
...
\draw[feedback]
    ([yshift=-8pt]core.east) --
    node[label, below=3pt] {reach}
    ([yshift=-8pt]frontier.west);
...
\draw[feedback]
    (uart.south) --
    node[label, pos=0.5, xshift=8pt] {UART, 115200 baud}
    (host.north);
...
(principle)
    at (4.8,-3.25)
    {\textbf{Key principle:} independent modules, streaming data flow.};
...
\begin{frame}{Frontier Engine — Row-Wise Algorithm}
  ...
  \begin{enumerate}
    ...
    \item \alert{Horizontal closure:} $\text{reach}[i] = \text{seed}[i] \lor (\text{open}[i] \land (\text{reach}[i{-}1] \lor \text{reach}[i{+}1]))$
  \end{enumerate}
...
\begin{frame}{Grid-Height Invariance \& Determinism}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=\textwidth]{throughput_invariance.png}\\
    \small Throughput vs grid height
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=\textwidth]{determinism_cv.png}\\
...
\begin{frame}{Threshold Estimation — Bootstrap}
  \centering
  \includegraphics[width=0.55\textwidth]{threshold_bootstrap.png}
