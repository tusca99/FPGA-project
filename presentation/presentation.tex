...
\begin{frame}{RNG Bank — Architecture}
  ...
  \vspace{0.1cm}
  \centering
  \includegraphics[width=0.75\textwidth]{sim_rng_warmup.png}
...
\begin{frame}{RNG Bank — Seeding \& Output}
  ...
  \begin{itemize}
    \item $N_{\text{rows}}$ 32-bit words
\item runs in streaming with core
  \end{itemize}
  \end{columns}

  \vspace{0.3cm}
    \centering
    \includegraphics[width=.75\textwidth]{sim_rng_valid.png}
...
\resizebox{\textwidth}{!}{
    \begin{tabular}{lcc}
      ...
      RNG autocorr. period & $\ge 2^{144}$ bits & Trivium keystream (no practical repetition) \\
